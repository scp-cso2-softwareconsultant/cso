    ...
    <v-card>
        <v-dialog v-model="dialog" width="700" class=' card p-md-5 pd-5'>
            <template v-slot:activator="{ on, attrs }">
                <v-btn class="ma-2" color="red lighten-2" dark block v-bind="attrs"  v-on="on">
                    View History 
                </v-btn>
            </template>
            <v-card>
                ...
                <div class="card-body">
                    <center>
                        <v-combobox
                            ...
                            hide-selected
                            small-chips
                            v-on:change="changeHistoryValue()"
                            v-model="date_value"
                            :items="date_selection"
                            label="PTD History List"
                        ></v-combobox>
                    </center>
                </div>
                <v-card class="ma-1 ">
                        <v-list-item>
                            <v-list-item-content>
                                ...
                                <div class="overline text-left">
                                    Days Completed: <strong> {{  editedItem. daysCompleted }} </strong>
                                </div>
                                <div class="overline text-left">
                                    Spent To Date: <strong> {{  editedItem.spent_to_date }} </strong>
                                </div>
                                <div class="overline text-left">
                                    Days Left : <strong> {{  editedItem.daysLeft }} </strong>
                                </div>
                                <div class="overline text-left">
                                    Percent Complete: <strong> {{  editedItem.percentComplete }} % </strong>
                                </div>
                                <div class="overline text-left">
                                    Burnrate : <strong> {{  editedItem.burnRate}} </strong>
                                </div>
                                <div class="overline text-left">
                                    Remaining : <strong> {{  editedItem.remaining }} </strong>
                                </div>
                            </v-list-item-content>
                        </v-list-item>
                </v-card>
        ...
        initialize(){
            axios.get('/get-project-tracking-document-history').then(response=>{
                ...
                for( const index in data ){
                    ...
                    date_selection.push({value,text})
