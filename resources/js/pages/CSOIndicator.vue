    ...
    <v-app>
                    ...
                    <v-container dense>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-select :items="category_items" v-model="editedItem.cso_category"
                                    :rules="[rules.required]" value="editedItem.cso_category" label="Category *" dense>
                                </v-select>
                            </v-col>
                        ...
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea auto-grow rows="1" :rules="[rules.required]"
                                    v-model="editedItem.cso_description" label="Description *" dense>
                                </v-textarea>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-combobox :readonly="detailsReadonly" :rules="[rules.required]"
                                    v-model="selected_cso_lead_organization" :items="lead_orgs"
                                    :search-input.sync="search" hide-selected label="Lead Organization *"
                                    hint="Hit backspace to delete selected Lead Org" multiple persistent-hint
                                    small-chips>
                        ...
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea v-model="editedItem.cso_act_no" :rules="[rules.required]"
                                    color="purple darken-2" :label="headers[0].text" required auto-grow rows="1" dense>
                                </v-textarea>
                            </v-col>
                        </v-row>
                        <v-row>
                            <v-col cols="12" sm="12" md="12" v-if="editedItem.created_at">
                                <v-select :items="status_list" v-model="editedItem.cso_status" label="Status" dense>
                                </v-select>
                            </v-col>
                        </v-row>
                        <v-row v-if="catSelectedTab === 'Output'" class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea auto-grow rows="1" v-model="editedItem.cso_remarks" label="Remarks" dense>
                                </v-textarea>
                            </v-col>
                        </v-row>
                        <v-row v-if="catSelectedTab === 'Output'" class="mt-0">
        ...
        <v-dialog v-model="dialogDelete" max-width="500px">
            <v-card color="blue-grey darken-3">
                <v-alert text color="red lighten-3" prominent type="error" icon="mdi-alert">Are you sure you want to
                    delete this item?</v-alert>
                ...
                <v-card-actions>
                    ...
                    <v-btn color="blue lighten-5" text @click="deleteItemConfirm" :loading="btnLoader">Yes</v-btn>
                    <v-spacer></v-spacer>
                </v-card-actions>
            </v-card>
        </v-dialog>
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-container dense>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-text-field v-model="editedSubItem.indicator_no" :rules="[rules.required]"
                                    :readonly="!isEditting" :label="this.subHeaders[0].text" dense></v-text-field>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea auto-grow rows="1" :rules="[rules.required]" :readonly="!isEditting"
                                    v-model="editedSubItem.indicator" label="Description *" dense></v-textarea>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-select :items="indicator_type_list" :readonly="!isEditting"
                                    v-model="editedSubItem.indicator_type" label="Type *" dense
                        ...
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea auto-grow rows="1" :rules="[rules.required]" :readonly="!isEditting"
                                    v-model="editedSubItem.data_source" label="Data Source *" dense></v-textarea>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-select :items="frequency_list" v-model="editedSubItem.frequency" label="Frequency *"
                                    dense :readonly="!isEditting" :rules="[rules.required]"></v-select>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea auto-grow rows="1" :rules="[rules.required]" :readonly="!isEditting"
                                    v-model="editedSubItem.unit_measure" label="Unit of Measure *" dense></v-textarea>
                            </v-col>
                        </v-row>
                        <v-row>
                            <v-col cols="12" sm="12" md="12">
                                <v-select :items="status_list" v-model="editedSubItem.indicator_status" label="Status"
                                    :readonly="!isEditting" dense></v-select>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea auto-grow rows="1" :readonly="!isEditting"
                                    v-model="editedSubItem.indicator_remarks" label="Remarks" dense></v-textarea>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-select :items="ppr_list" v-model="editedSubItem.ppr" label="PPR *" dense
                                    :readonly="!isEditting" :rules="[rules.required]"></v-select>
                            </v-col>
                        </v-row>

                        <v-row class="mt-0" v-if="detailsReadonly">
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea auto-grow rows="1" v-model="editedSubItem.baseline_date"
                                    label="Baseline Date" dense readonly></v-textarea>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0" v-if="!detailsReadonly">
                            <v-col cols="12" sm="12" md="12">
                                <v-menu ref="modelBaselineDate" v-model="modelBaselineDate"
                                    :close-on-content-click="false" transition="scale-transition" offset-y
                                    max-width="290px" min-width="auto" dense>
                                    <template v-slot:activator="{ on, attrs }">
                                        <v-text-field v-model="editedSubItem.baseline_date" label="Baseline Date"
                                            readonly persistent-hint append-outer-icon="mdi-calendar" v-bind="attrs"
                                            v-on="on" dense></v-text-field>
                                    </template>
                                    <v-date-picker v-model="editedSubItem.baseline_date" no-title
                                        @input="modelBaselineDate = false"></v-date-picker>
                                </v-menu>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea auto-grow rows="1" :readonly="detailsReadonly"
                                    v-model="editedSubItem.baseline_value" label="Baseline Value" dense></v-textarea>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0" v-if="detailsReadonly">
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea auto-grow rows="1" v-model="editedSubItem.target_date" label="Target Date"
                                    dense readonly></v-textarea>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0" v-if="!detailsReadonly">
                            <v-col cols="12" sm="12" md="12">
                                <v-menu ref="modelTargetDate" v-model="modelTargetDate" :close-on-content-click="false"
                                    transition="scale-transition" offset-y max-width="290px" min-width="auto" dense>
                                    <template v-slot:activator="{ on, attrs }">
                                        <v-text-field v-model="editedSubItem.target_date" label="Target Date" readonly
                                            persistent-hint append-outer-icon="mdi-calendar" v-bind="attrs" v-on="on"
                                            dense></v-text-field>
                                    </template>
                                    <v-date-picker v-model="editedSubItem.target_date" no-title
                                        @input="modelTargetDate = false"></v-date-picker>
                                </v-menu>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea auto-grow rows="1" :readonly="detailsReadonly"
                                    v-model="editedSubItem.target_value" label="Target Value" dense></v-textarea>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0" v-if="detailsReadonly">
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea auto-grow rows="1" v-model="editedSubItem.actual_date" label="Actual date"
                                    dense readonly></v-textarea>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0" v-if="!detailsReadonly">
                            <v-col cols="12" sm="12" md="12">
                                <v-menu ref="modelTargetDate" v-model="modelActualDate" :close-on-content-click="false"
                                    transition="scale-transition" offset-y max-width="290px" min-width="auto" dense>
                                    <template v-slot:activator="{ on, attrs }">
                                        <v-text-field v-model="editedSubItem.actual_date" label="Actual Date" readonly
                                            persistent-hint append-outer-icon="mdi-calendar" v-bind="attrs" v-on="on"
                                            dense></v-text-field>
                                    </template>
                                    <v-date-picker v-model="editedSubItem.actual_date" no-title
                                        @input="modelActualDate = false"></v-date-picker>
                                </v-menu>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0">
                                        ...
                                        <v-tooltip bottom>
                                            <template v-slot:activator="{ on, attrs }">
                                                <v-chip class="ma-2" color="indigo darken-3" outlined v-on="on">
                                                    <v-icon left> mdi-file </v-icon>
                                                    <a :href="`/downloadMov/?file_name=${file_name}&category=${catSelectedTab}`"
                                                        target="blank">
                                                        {{ limitChipName(file_name)}}
                                                    </a>
                                                </v-chip>
                ...
                <v-card-actions class="d-flex  blue-grey darken-3 white--text" v-if="isEditting">
                    <v-spacer></v-spacer>
                    <v-btn color="blue lighten-5" text @click="closeSub" dense>
                        Cancel
                    </v-btn>
                    <v-btn color="blue lighten-5" text @click="saveSub" dense :loading="btnLoader">
                        Save
                    </v-btn>
                </v-card-actions>
            </v-card>
        ...
        <div class="d-flex m-4 flex-between align-items-center">
            ...
            <v-col class="mx-3 btn" cols="2" v-for="(stats, idx) in status_list" :key="idx">
                <v-progress-linear class="rounded-top rounded-bottom text-white text-center" height="35"
                    :color="getColor(stats.text)" @click="setFilterByStatus(stats.text)" value="100">{{ stats.value }}
                </v-progress-linear>
            </v-col>
        </div>
        <br />
        <template>
            <v-card>
                <v-tabs v-model="tabCategory">
                    <v-tab v-for="item in category_tabs" :key="item.value" @click="getIndicator(item.value)">
                        {{ item.text }}
                    </v-tab>
                </v-tabs>
                <v-tabs-items v-model="tabCategory">
                    <v-tab-item v-for="item in category_tabs" :key="item.value">
                        <v-card>
                            <v-data-table v-if="catSelectedTab === 'Output'" :headers="headers" :items="indicators_list"
                                ...
                                <template v-slot:top>
                                    <v-toolbar class="py-4" flat dense>
                                        <v-text-field v-model="searchBy" append-icon="mdi-magnify" label="Search"
                                            single-line hide-details></v-text-field>
                                        <v-col cols="3">
                                            <v-combobox v-model="selected_filter_item" :items="filter_items"
                                                label="Filter By" @change="initFilter2Items" chips dense></v-combobox>
                                        </v-col>
                                        <v-col v-if="selected_filter_item != 'All'" cols="4">
                                            <v-combobox v-model="selected_filter_item2" :items="filter_items2"
                                                :label="filter2_label" @change="initialize" dense chips></v-combobox>
                                        </v-col>
                                        <v-btn v-show="crud_guard.create" color="lightgray" class="mb-2"
                                            @click="newActivity">
                                            New
                                            <v-icon color="green">mdi-plus-thick</v-icon>
                                        </v-btn>
                                        &nbsp;&nbsp;
                                        <v-btn v-show="crud_guard.export" color="lightgray" class="mb-2"
                                            :loading="btnLoader" @click="exportExcel('CSOIndicator', item.value)">
                                            Export
                                ...
                                <template v-slot:item.cso_status="{ item }">
                                    <v-progress-linear class="rounded-top text-white text-center" height="23"
                                        value="100" :color="getColor(item.cso_status)"></v-progress-linear>
                                    <!-- <v-progress-linear
                                        indeterminate
                                ...
                                <template v-slot:item.actions="{ item }">
                                    <v-icon v-show="crud_guard.update" small class="mr-2" @click="editItem(item)"
                                        color="blue darken-2" data-toggle="tooltip" data-placement="top"
                                        title="Edit CSO2 Indicator">
                                        mdi-pencil
                                    </v-icon>
                                    <v-icon v-show="crud_guard.delete" small @click="deleteItem(item)" color="red"
                                        data-toggle="tooltip" data-placement="top" title="Delete This Item">
                                        mdi-delete
                                    </v-icon>
                                </template>
                                    ...
                                    <td :colspan="headers.length">
                                        <v-data-table :headers="subHeaders" :items="item.subItems" class="elevation-1">
                                            <template v-slot:item.indicator_status="{ item }">
                                                <v-progress-linear class="rounded-top text-white text-center"
                                                    height="23" value="100" :color="getColor(item.indicator_status)">
                                                </v-progress-linear>
                                                <!-- <v-progress-linear
                                                    indeterminate
                                                    class="rounded-bottom text-white text-center"
                                            ...
                                            <template v-slot:item.actions="{ item }">
                                                <v-icon v-show="crud_guard.view" small class="mr-2"
                                                    @click="detailsSubItem(item)" color="blue" data-toggle="tooltip"
                                                    data-placement="top" title="Indicator Details">
                                                    mdi-information-outline
                                                </v-icon>
                                                <br />
                                                <v-icon v-show="crud_guard.update" small class="mr-2"
                                                    @click="editSubItem(item)" color="blue darken-2"
                                                    data-toggle="tooltip" data-placement="top"
                                                    title="Edit Indicator Details">
                                                    mdi-pencil
                                                </v-icon>
                                                <br />
                                                <v-icon v-show="crud_guard.delete" small @click="deleteSubItem(item)"
                                                    color="red" data-toggle="tooltip" data-placement="top"
                            ...
                            <v-data-table v-else :headers="headers" :items="indicators_list"
                                :single-expand="singleExpand" :expanded.sync="expanded" :search="searchBy"
                                item-key="cso_indicator_id" :loading="loadCSOIndicator" show-expand class="elevation-1">
                                <template v-slot:top>
                                    <v-toolbar class="py-4" flat dense>
                                        <v-text-field v-model="searchBy" append-icon="mdi-magnify" label="Search"
                                            single-line hide-details></v-text-field>
                                ...
                                <template v-slot:item.actions="{ item }">
                                    <v-icon v-show="crud_guard.create" small class="mr-2" @click="addSubItem(item)"
                                        color="green" data-toggle="tooltip" data-placement="top"
    ...
    export default {
        ...
        methods: {
            ...
            getFilteredIndicator() {
                axios
                    .post("/cso-indicator-list", {
                        category: this.catSelectedTab
                    })
                    .then((response) => {
                        this.indicators_list = this.filterData(response);
                        this.loadCSOIndicator = false;
            ...
            editItem(item) {
                ...
                this.selected_cso_lead_organization = [{
                    text: item.cso_lead_organization,
                    value: item.cso_lead_organization
                }]
            ...
            async save() {
                ...
                if (this.selected_cso_lead_organization.length === 0) {
                    ...
                    if (Object.prototype.toString.call(this.selected_cso_lead_organization[0]) ===
                        '[object Object]') {
