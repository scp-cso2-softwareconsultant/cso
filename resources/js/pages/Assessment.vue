    ...
    <v-app>
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-container dense>
                        ...
                        <v-row class="mt-0" v-if="detailsReadonly">
                            <v-col cols="12" sm="12" md="12">
                                <v-text-field v-model="editedItem.org_address" label="Address *" dense readonly>
                                </v-text-field>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0" v-if="!detailsReadonly">
                            <v-col cols="12" sm="12" md="12">
                                <v-select :items="tool_list" v-model="editedItem.tool_used" label="Tool Used *" dense
                                    :rules="[rules.required]"></v-select>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0" v-if="detailsReadonly">
                            <v-col cols="12" sm="12" md="12">
                                <v-text-field v-model="editedItem.tool_used" label="Tool Used *" dense readonly>
                                </v-text-field>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-text-field :readonly="detailsReadonly" v-model="editedItem.final_score"
                                    label="Final Score" dense></v-text-field>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-text-field :readonly="detailsReadonly" v-model="editedItem.conducted_by"
                                    label="Conducted By" dense></v-text-field>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0" v-if="!detailsReadonly">
                            <v-col cols="12" sm="12" md="12">
                                <v-menu ref="modelAssessmentDate" v-model="modelAssessmentDate"
                                    :close-on-content-click="false" transition="scale-transition" offset-y
                                    max-width="290px" min-width="auto" dense>
                                    <template v-slot:activator="{ on, attrs }">
                                        <v-text-field v-model="editedItem.assessment_date" label="Assessment Date"
                                            persistent-hint append-outer-icon="mdi-calendar" v-bind="attrs" v-on="on"
                                            dense></v-text-field>
                                    </template>
                                    <v-date-picker v-model="editedItem.assessment_date" no-title
                                        @input="modelAssessmentDate = false" dense></v-date-picker>
                                </v-menu>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0" v-if="detailsReadonly">
                            <v-col cols="12" sm="12" md="12">
                                <v-text-field v-model="editedItem.assessment_date" label="Assessment Date" dense
                                        ...
                                        <v-tooltip top>
                                            <template v-slot:activator="{ on, attrs }">
                                                <v-chip class="ma-2" color="indigo darken-3" outlined v-on="on">
                                                    <v-icon left> mdi-file </v-icon>
                                                    <a :href="`/downloadAssessMov/?file_name=${editedItem.mov}`"
                                                        target="blank">
                                                        {{ limitChipName(file_name) }}
                                                    </a>
                                                </v-chip>
        ...
        <v-dialog v-model="dialogDelete" max-width="500px">
            <v-card color="blue-grey darken-3">
                <v-alert text color="red lighten-3" prominent type="error" icon="mdi-alert">
					Are you sure you want to delete this item?
				</v-alert>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="blue darken-1" text @click="closeDelete">No</v-btn>
                    <v-btn color="blue darken-1" text @click="deleteItemConfirm" :loading="btnLoader">Yes</v-btn>
                    <v-spacer></v-spacer>
                </v-card-actions>
            </v-card>
        </v-dialog>
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-container dense>
                        <v-row class="mt-0">
                            <v-col cols="12" sm="12" md="12">
                                <v-text-field v-model="editedSubItem.sub_domain" :rules="[rules.required]"
                                    :readonly="detailsReadonly" label="Domain *" dense></v-text-field>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0" v-if="!detailsReadonly">
                            <v-col cols="12" sm="12" md="12">
                                <v-text-field v-model="editedSubItem.rating" :rules="[rules.required]"
                                    :readonly="detailsReadonly" label="Rating *" dense></v-text-field>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0" v-if="detailsReadonly">
                            <v-col cols="12" sm="12" md="12">
                                <v-text-field v-model="editedSubItem.rating" label="Rating *" dense readonly>
                                </v-text-field>
                            </v-col>
                        </v-row>
                        <v-row class="mt-0">
        ...
        <v-card>
            <v-data-table :headers="headers" :items="lroList" :single-expand="singleExpand" :expanded.sync="expanded"
                :search="filters.filter_items[filters.filter_items_active].value" :custom-filter="filterItems"
                :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :loading="loadLROAssessment"
                item-key="lro_assessment_id" class="elevation-1" show-expand>
                <template v-slot:top>
                    <v-toolbar flat>
                        <v-spacer></v-spacer>
                        <v-btn v-if="crud_guard.create" color="lightgray" class="mb-2" @click="newAsessment">
                            New
                            <v-icon color="green">mdi-plus-thick</v-icon>
                        </v-btn>
                        &nbsp;&nbsp;
                        <v-btn color="lightgray" v-if="crud_guard.export" class="mb-2" :loading="btnLoader"
                    ...
                    <v-row no-gutters style="flex-wrap: nowrap">
                        ...
                        <v-col cols="4" style="min-width: 100px; max-width: 50%" class="flex-grow-1 flex-shrink-0">
                            <v-text-field v-model="filters.filter_items['domain'].value"
                                :label="filters.filter_items['domain'].text" @input="changeFilterActiveValue('domain')"
                                append-icon="mdi-magnify" outlined hide-details></v-text-field>
                        </v-col>
                        <v-col cols="1" style="min-width: 100px; max-width: 70%" class="flex-grow-1 flex-shrink-0">
                            <v-select v-model="filters.filter_items['tool_used'].value"
                                :label="filters.filter_items['tool_used'].text"
                                :items="filters.filter_items['tool_used'].multiple_selection"
                                @input="changeFilterActiveValue('tool_used')" outlined hide-details></v-select>
                        </v-col>
                    </v-row>
                    <v-row no-gutters style="flex-wrap: nowrap" class="my-3">
                        <v-col cols="3" style="min-width: 50px" class="flex-grow-0 flex-shrink-1">
                            <div v-if="filters.filter_items['final_score_selection'].value != 'range'">
                                <v-text-field v-model="filters.filter_items['final_score'].value"
                                    :label="filters.filter_items['final_score'].text"
                                    @input="changeFilterActiveValue('final_score')" :rules="[rules.number]" outlined
                                    hide-details></v-text-field>
                            </div>
                            <div v-else>
                                <v-text-field v-model="filters.filter_items['final_score_min'].value"
                                    :label="filters.filter_items['final_score_min'].text"
                                    @input="changeFilterActiveValue('final_score_min')" :rules="[rules.number]" outlined
                                    hide-details></v-text-field>
                                <v-text-field v-model="filters.filter_items['final_score_max'].value"
                                    :label="filters.filter_items['final_score_max'].text"
                                    @input="changeFilterActiveValue('final_score_max')" :rules="[rules.number]" outlined
                                    hide-details></v-text-field>
                            </div>
                        </v-col>
                        <v-col cols="2" class="flex-grow-1 flex-shrink-0">
                            <v-select v-model="filters.filter_items['final_score_selection'].value"
                        ...
                        <v-col cols="1" style="min-width: 100px; max-width: 70%" class="flex-grow-1 flex-shrink-0">
                            <v-select v-model="filters.filter_items['status'].value"
                                :label="filters.filter_items['status'].text"
                                :items="filters.filter_items['status'].multiple_selection"
                                @input="changeFilterActiveValue('status')" outlined hide-details></v-select>
                        </v-col>
                    </v-row>
                    <v-row no-gutters style="flex-wrap: nowrap">
                        <v-col cols="4" style="min-width: 100px; max-width: 50%" class="flex-grow-1 flex-shrink-0">
                            <v-text-field v-model="filters.filter_items['conducted_by'].value"
                                :label="filters.filter_items['conducted_by'].text"
                                @input="changeFilterActiveValue('conducted_by')" append-icon="mdi-magnify" outlined
                                hide-details></v-text-field>
                        </v-col>
                        <v-col cols="4" style="min-width: 50px" class="flex-grow-0 flex-shrink-1">
                            <div v-if="filters.filter_items['displayDate_selection'].value != 'range' ">
                                <v-menu v-model="filters.filter_items['displayDate'].fromDateMenu"
                                    :close-on-content-click="false" transition="scale-transition" offset-y
                                    max-width="290px" min-width="290px" hide-details>
                                    <template v-slot:activator="{ on }">
                                        <v-text-field readonly v-on="on"
                                            :value="filters.filter_items['displayDate'].value"
                                            :label="filters.filter_items['displayDate'].text"
                                            @input="changeFilterActiveValue('displayDate')" append-icon="mdi-magnify"
                                            outlined hide-details></v-text-field>
                                    </template>
                                    <v-date-picker locale="en-in" hide-details
                                        v-model="filters.filter_items['displayDate'].value" 
										@input="filters.filter_items['displayDate'].fromDateMenu = false;changeFilterActiveValue('displayDate');"></v-date-picker>
                                </v-menu>
                            </div>
                            <div v-else>
                                <v-menu v-model="filters.filter_items['displayDate_min'].fromDateMenu"
                                    :close-on-content-click="false" transition="scale-transition" offset-y
                                    max-width="290px" min-width="290px" hide-details>
                                    <template v-slot:activator="{ on }">
                                        <v-text-field readonly v-on="on"
                                    ...
                                    <v-date-picker locale="en-in"
                                        v-model="filters.filter_items['displayDate_min'].value" 
										@input="filters.filter_items['displayDate_min'].fromDateMenu = false;changeFilterActiveValue('displayDate_min'); "></v-date-picker>
                                </v-menu>
                                <v-menu v-model="filters.filter_items['displayDate_max'].fromDateMenu"
                                    :close-on-content-click="false" transition="scale-transition" offset-y
                                    max-width="290px" min-width="290px" hide-details>
                                    <template v-slot:activator="{ on }">
                                        <v-text-field readonly v-on="on"
                                            ...
                                            append-icon="mdi-magnify" outlined></v-text-field>
                                    </template>
                                    <v-date-picker v-model="filters.filter_items['displayDate_max'].value" 
										@input=" filters.filter_items['displayDate_max'].fromDateMenu = false;changeFilterActiveValue('displayDate_max'); "	hide-details>
									</v-date-picker>
                                </v-menu>
                            </div>
                        </v-col>
                        <v-col cols="2" class="flex-grow-1 flex-shrink-0">
                            <v-select v-model="filters.filter_items['displayDate_selection'].value"
                                :label="filters.filter_items['displayDate_selection'].text" :items="filters.filter_items['displayDate_selection'].multiple_selection" 
								@input="changeFilterActiveValue('displayDate_selection')" 
								v-on:change=" if (filters.filter_items['displayDate_selection'].value == '')filters.filter_items['displayDate'].value = '';" outlined>
							</v-select>
                        </v-col>
                    </v-row>
                </template>
                ...
                <template v-slot:item.actions="{ item }">
                    <v-icon v-if="crud_guard.create" small class="mr-2" @click="addSubItem(item)" color="green"
                        data-toggle="tooltip" data-placement="top" title="New Domain">
                        mdi-sticker-plus-outline
                    </v-icon>
                    <v-icon v-if="crud_guard.view" small class="mr-2" @click="detailsItem(item)" color="blue"
                        data-toggle="tooltip" data-placement="top" title="Assessment Details">
                        mdi-information-outline
                    </v-icon>
                    <v-icon v-if="crud_guard.update" small class="mr-2" @click="editItem(item)" color="blue darken-2"
                        data-toggle="tooltip" data-placement="top" title="Edit Assessment">
                        mdi-pencil
                    </v-icon>
                    <v-icon v-if="crud_guard.delete" small @click="deleteItem(item)" color="red" data-toggle="tooltip"
                        data-placement="top" title="Delete This Item">
                        mdi-delete
                    </v-icon>
                </template>
                <template v-slot:expanded-item="{ headers, item }">
                    <td :colspan="headers.length">
                        <v-data-table :headers="subHeaders" :items="item.subItems" class="elevation-1">
                            ...
                            <template v-slot:item.actions="{ item }">
                                <v-icon v-if="crud_guard.update" small class="mr-2" @click="editSubItem(item)"
                                    color="blue darken-2" data-toggle="tooltip" data-placement="top"
                                    title="Edit Subdomain">
                                    mdi-pencil
                                </v-icon>
                                <v-icon v-if="crud_guard.delete" small @click="deleteSubItem(item)" color="red"
